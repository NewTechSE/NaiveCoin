...
const validateTransaction = (transaction: Transaction, aUnspentTxOuts: UnspentTxOut[]): boolean => {
  ...
  const hasValidTxIns: boolean = transaction.txIns.map(txIn => validateTxIn(txIn, transaction, aUnspentTxOuts)).reduce((a, b) => a && b, true);
  ...
  const totalTxInValues: number = transaction.txIns.map(txIn => getTxInAmount(txIn, aUnspentTxOuts)).reduce((a, b) => a + b, 0);
